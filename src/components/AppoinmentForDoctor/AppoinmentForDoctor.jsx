...
const AppoinmentForDoctor = ({ doctorId }) => {
    ...
    useEffect(() => {
        const doctorFind = async () => {
            try {
                const res = await axios.get(`${process.env.NEXT_BASE_URL}/appoinment/doctor-query?name=${isDoctorName}`)
    ...
    const { data: doctorData = [] } = useQuery({
        queryKey: ["doctorData"],
        queryFn: async () => {
            const res = await axios.get(`${process.env.NEXT_BASE_URL}/appoinment/doctor_get?id=${doctorId}`)
            return res.data
        }
    })
    ...
    const { data: apponmentData = [] } = useQuery({
        ...
        queryFn: async () => {
            const res = await axios.get(`${process.env.NEXT_BASE_URL}/appoinment/doctor_appoinment`)
            return res.data
        }
    })
    ...
    const onSubmit = async (data) => {
        ...
        if (isPayment) {
            axios.post(`${process.env.NEXT_BASE_URL}/appoinment/appoinment_payment`, appoinmentData)
                .then(res => {
        ...
        if (isNotPayment) {
            const res = await axios.post(`${process.env.NEXT_BASE_URL}/appoinment/doctor_appoinment`, appoinmentData)
